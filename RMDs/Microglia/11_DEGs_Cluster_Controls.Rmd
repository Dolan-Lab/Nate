...
Isolate the control samples for a DEG list to use in the GSEA analysis
```{r}
mg <- subset(mg, Genotype == "ATF3 fl/fl hom")
```

Pseudobulk and check the object to ensure that it still has the relevant data
```{r}
y <- Seurat2PB(mg, sample = "animal_id", cluster = "seurat_clusters") #Confirmed this is Sum of raw count
head(y)
```
...
design <- model.matrix(~ cluster + animal) #This is pseudobulk specific matrix design because different clusters can come from the same animal and we do not want to compare timepoint and condition in this analysis
...
comparison_index <- which(sapply(qlf, function(x) x$comparison == "cluster4_vs_others")) # You need to define this based on your comparison naming convention
```
...
temp <- geneList %>% subset(geneList[,2] == "cluster4_vs_others")
...
Heatmap(mat_averages, column_title = "DEGs by cluster", cluster_columns = F, cluster_rows =F, show_column_names = TRUE,show_row_names = F ,show_row_dend = FALSE, show_column_dend =TRUE,name = " ")+rowAnnotation(link = anno_mark(at = which(rownames(mat_averages) %in% Genes.to.plot),labels = rownames(mat_averages)[which(rownames(mat_averages) %in% Genes.to.plot)],labels_gp = gpar(fontsize = 10), padding = unit(1, "mm")))
```


```{r}
write.csv(macs.markers, "/Users/Nate/Desktop/Mikes_ATF3_Data/CSVs/Control_DEGs.csv")
```
