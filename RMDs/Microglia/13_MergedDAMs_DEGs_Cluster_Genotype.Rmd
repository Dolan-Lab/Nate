---
title: "13_MergedDAMs_DEGs_Cluster_Genotype"
...
```{r}
for (i in 1:length(mg$seurat_clusters)) {
  if(mg$seurat_clusters[i] == "3"){
    mg$seurat_clusters[i] <- "2" 
  }
}
...
```{r}  
  ...
  #Initialize the upset plot to plot by cluster 
  macs.markers.filtered<-filter(macs.markers, FDR < 0.05)
  macs.markers.filtered<-mutate(macs.markers.filtered, Positive_gene=paste0(logFC>0,"_", gene)) #Distinguish between positive and negatively enriched genes so you can count both
  de_gs_by_con<-split(macs.markers.filtered, macs.markers.filtered$clusterID)
  ...
  #Code for labelling genes in Volcano plot 
  geneList <- df["Gene", "Comparison"]
   topLFC <- macs.markers %>% subset(clusterID == 2) %>% arrange(desc(logFC))
   topFDR <- macs.markers %>% subset(clusterID == 2) %>% arrange(FDR)
   topHits <- c()
   for(b in 1:length(topLFC$gene)){
    ...
    for(j in 1:15){
      geneList <- rbind(geneList, list(topHits[j], 2))
    }
  ...
  cluster2degs <- macs.markers %>% subset(clusterID == 2)
  ...
  volcano_data$Gene <- cluster2degs$gene
    ...
    temp <- geneList %>% subset(geneList[,2] == 2)
    ...
    ggplot(data=volcano_data, aes(x=logFC, y=PValue)) +
      geom_point(aes(color=Significant), alpha=0.4, shape=16) + # Apply color mapping only here. Need to specify shape=16 due to bug in ggplot2 that does not   apply alpha to points
      theme(axis.text=element_text(size=14), axis.title=element_text(size=14,face="bold"), plot.title = element_text(hjust = 0.5)) +
      geom_text_repel(aes(label=Label), size = 4.5, max.overlaps = Inf, min.segment.length = 0) +
      scale_color_manual(values=c("darkblue", "red")) +
      theme_classic()
```
...
write.csv(macs.markers, "DEGs_by_Cluster_by_Genotype_MergedDAMs.csv") 
```
...
```{r}
setwd("/Users/nate/Desktop/Mikes_ATF3_Data/CSVs")
write.csv(macs.markers, "DEGs_by_Cluster_by_Genotype_MergedDAMs_KOselected.csv") 
```


```{r}
for (i in 1:length(mg$annotations)) {
  if(mg$annotations[i] == "Lpl DAMs 1" || mg$annotations[i] == "Lpl DAMs 2"){
    mg$annotations[i] <- "Lpl DAMs" 
  }
  else if(mg$annotations[i] == "Undefined"){
    mg$annotations[i] <- "Transitory"
  }
}

Idents(mg) <- mg$annotations
DimPlot(mg, label = T, raster=FALSE) + plot_annotation("Microglia Transcriptional States")
```
