...
```{r}  
  ...
  geneList <- df["Gene", "Comparison"]
   topLFC <- macs.markers %>% subset(clusterID == 5) %>% arrange(desc(logFC))
   topFDR <- macs.markers %>% subset(clusterID == 5) %>% arrange(FDR)
   topHits <- c()
   for(b in 1:length(topLFC$gene)){
    ...
    for(j in 1:15){
      geneList <- rbind(geneList, list(topHits[j], 5))
    }
  ...
  cluster2degs <- macs.markers %>% subset(clusterID == 5)
  ...
  volcano_data$Gene <- cluster2degs$gene
    ...
    temp <- geneList %>% subset(geneList[,2] == 5)
...
write.csv(macs.markers, "DEGs_by_Cluster_by_Genotype.csv") 
```
