---
title: "GSEA_ControlsDEGs_GroundTruth"
...
Controls.DEGs<-read.csv("Control_DEGs.csv") #less granular breakdown
Controls.DEGs<-filter(Controls.DEGs, FDR<0.05)
Controls.DEGs<-filter(Controls.DEGs, logFC>0) #Just positively expressed genes
#Mike.DEGs<-filter(Mike.DEGs, cluster_name!="Myeloid_CD300E", cluster_name!="Macrophage_CD200R1")
Controls.DEGs<-arrange(Controls.DEGs, comparison, desc(logFC)) #This ordering is critical to get correct results 
#Vahid.DEGs<-filter(Vahid.DEGs, avg_logFC>0.1)

Controls.DEGs<-split(Controls.DEGs,Controls.DEGs$comparison)
Controls.DEGs.List<-lapply(Controls.DEGs, extract_gene)

#Use Babelgene to convert mouse to human 
lapply(Controls.DEGs.List, length)
...
for(i in 1:num_clusters) { 
 ...
 fgseaRes.temp <- fgsea(pathways = Controls.DEGs.List ,nperm = 10000
